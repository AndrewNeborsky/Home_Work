...
def fun(value, action):
    d = {'sum': reduce(lambda a, x: a + x, value),
         ...
         'odds': [value[j] for j in range(len(value)) if value[j] % 2 != 0],
         'evens': [value[j] for j in range(len(value)) if value[j] % 2 != 0],
         'simples': [value[j] for j in range(len(value)) if value[j] in {1, 2, 3, 5, 7}]}
